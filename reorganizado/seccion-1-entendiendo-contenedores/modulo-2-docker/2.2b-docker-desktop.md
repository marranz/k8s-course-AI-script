# 2.2b Docker Desktop: Guía Completa

## ¿Qué es Docker Desktop?

Docker Desktop es una aplicación nativa para Windows, macOS y Linux que proporciona:
- Docker Engine empaquetado y listo para usar
- Interfaz gráfica (GUI) para gestionar contenedores e imágenes
- Docker CLI integrado
- Docker Compose incluido
- Kubernetes local (opcional)
- Actualizaciones automáticas
- Integración con el sistema operativo

## Ventajas de Docker Desktop

### Para Desarrollo Local
- ✅ Instalación simplificada (un solo instalador)
- ✅ No necesitas configurar manualmente el daemon
- ✅ Gestión visual de recursos
- ✅ Integración con IDEs (VS Code, IntelliJ)
- ✅ Compartición de archivos automática
- ✅ Networking configurado automáticamente

### Características Destacadas
- Dashboard visual intuitivo
- Gestión de recursos (CPU, RAM, disco)
- Kubernetes con un click
- Docker Compose UI integrada
- Extensiones (Beta)
- Sincronización de credenciales Docker Hub

## Instalación por Sistema Operativo

### Windows

#### Requisitos Previos
- Windows 10 64-bit: Pro, Enterprise o Education (Build 19041 o superior)
- O Windows 11 64-bit: Home o Pro (Build 22000 o superior)
- Habilitar WSL 2 (Windows Subsystem for Linux)
- Virtualización habilitada en BIOS

#### Pasos de Instalación

1. **Descargar Docker Desktop**
   - Visita: https://www.docker.com/products/docker-desktop
   - Click en "Download for Windows"

2. **Instalar WSL 2** (si no está instalado)
   ```powershell
   # Ejecutar en PowerShell como Administrador
   wsl --install

   # Reiniciar el sistema
   # Después del reinicio, establecer WSL 2 como predeterminado
   wsl --set-default-version 2
   ```

3. **Ejecutar el Instalador**
   - Doble click en `Docker Desktop Installer.exe`
   - Marcar "Use WSL 2 instead of Hyper-V" (recomendado)
   - Click "Ok"
   - Esperar la instalación
   - Click "Close and restart"

4. **Primera Ejecución**
   - Docker Desktop se iniciará automáticamente
   - Acepta el acuerdo de servicio
   - Opcionalmente, inicia sesión con tu Docker ID

5. **Verificar Instalación**
   ```powershell
   # En PowerShell o CMD
   docker --version
   docker-compose --version
   docker run hello-world
   ```

#### Troubleshooting Windows
- **Error: WSL 2 installation is incomplete**
  - Instala el kernel update: https://aka.ms/wsl2kernel

- **Error: Hardware assisted virtualization**
  - Habilita virtualización en BIOS (VT-x/AMD-V)

- **Docker Desktop won't start**
  - Verifica que WSL 2 esté corriendo: `wsl --status`
  - Reinicia el servicio: Settings → Reset → Restart Docker Desktop

### macOS

#### Requisitos Previos
- macOS 11 Big Sur o superior
- Mac con chip Apple Silicon (M1/M2) o Intel
- Mínimo 4GB RAM

#### Pasos de Instalación

1. **Descargar Docker Desktop**
   - Visita: https://www.docker.com/products/docker-desktop
   - Descarga la versión correcta:
     - "Docker Desktop for Mac with Apple Silicon"
     - "Docker Desktop for Mac with Intel chip"

2. **Instalar**
   - Abre el archivo `Docker.dmg` descargado
   - Arrastra el icono de Docker a la carpeta Applications
   - Abre Docker desde Applications

3. **Permisos**
   - macOS pedirá tu contraseña para instalar componentes
   - Acepta los permisos necesarios

4. **Primera Ejecución**
   - Docker Desktop aparecerá en la barra de menú (🐳)
   - Acepta el acuerdo de servicio
   - Completa el tutorial opcional

5. **Verificar Instalación**
   ```bash
   # En Terminal
   docker --version
   docker-compose --version
   docker run hello-world
   ```

#### Troubleshooting macOS
- **Docker Desktop is starting loop**
  - Borra configuraciones: `rm -rf ~/Library/Group\ Containers/group.com.docker`
  - Reinicia Docker Desktop

- **Slow performance on M1/M2**
  - Asegúrate de usar la versión Apple Silicon
  - Habilita VirtioFS en Settings → Experimental Features

### Linux

#### Requisitos Previos
- Ubuntu 20.04/22.04, Debian 11, Fedora 37/38
- Arquitectura x86_64/amd64
- KVM support
- QEMU 5.2 o superior

#### Pasos de Instalación (Ubuntu/Debian)

1. **Descargar Docker Desktop**
   ```bash
   # Descargar el paquete .deb
   wget https://desktop.docker.com/linux/main/amd64/docker-desktop-<version>-amd64.deb
   ```

2. **Instalar Dependencias**
   ```bash
   sudo apt-get update
   sudo apt-get install -y \
     ca-certificates \
     curl \
     gnupg \
     lsb-release
   ```

3. **Instalar Docker Desktop**
   ```bash
   sudo apt-get install ./docker-desktop-<version>-amd64.deb
   ```

4. **Iniciar Docker Desktop**
   ```bash
   systemctl --user start docker-desktop

   # O desde el menú de aplicaciones
   ```

5. **Verificar Instalación**
   ```bash
   docker --version
   docker-compose --version
   docker run hello-world
   ```

#### Troubleshooting Linux
- **Permission denied on /var/run/docker.sock**
  ```bash
  sudo usermod -aG docker $USER
  newgrp docker
  ```

## Tour de la Interfaz Gráfica

### Dashboard Principal

Cuando abres Docker Desktop, ves:

```
┌─────────────────────────────────────────────────┐
│  Docker Desktop                    [⚙️] [👤]    │
├─────────────────────────────────────────────────┤
│  🐋 Containers (3)                              │
│  📦 Images (8)                                  │
│  📚 Volumes (2)                                 │
│  🔌 Dev Environments                            │
│  🧩 Extensions                                  │
└─────────────────────────────────────────────────┘
```

### Pestaña Containers

**Vista de contenedores activos:**
- Nombre del contenedor
- Estado (Running, Stopped)
- Puerto mapeado
- Botones de acción:
  - ▶️ Start / ⏸️ Stop
  - 🔄 Restart
  - 🗑️ Delete
  - 📄 Logs
  - 🖥️ CLI (abrir terminal)
  - 📊 Stats (estadísticas)

**Acciones comunes:**

1. **Ver logs en tiempo real**
   - Click en el contenedor
   - Pestaña "Logs"
   - Scroll automático activado

2. **Abrir terminal dentro del contenedor**
   - Click en el contenedor
   - Pestaña "Terminal" o botón CLI
   - Ejecuta comandos interactivos

3. **Ver estadísticas**
   - Pestaña "Stats"
   - CPU, memoria, red, disco en tiempo real

### Pestaña Images

**Gestión de imágenes:**
- Lista de imágenes locales
- Tamaño de cada imagen
- Etiquetas (tags)
- Fecha de creación

**Acciones:**
- 🚀 Run (crear contenedor desde imagen)
- 🗑️ Delete
- 🔍 Inspect (ver detalles)
- 🏷️ Tag (etiquetar imagen)
- ⬆️ Push (subir a registry)

**Buscar imágenes en Docker Hub:**
- Barra de búsqueda superior
- Resultados de Docker Hub
- Pull directo desde la UI

### Pestaña Volumes

**Gestión de volúmenes:**
- Lista de volúmenes creados
- Tamaño utilizado
- Contenedores que los usan

**Acciones:**
- 🗑️ Delete
- 🔍 Inspect
- 📁 Browse data (explorar archivos del volumen)

### Dev Environments (Beta)

Entornos de desarrollo colaborativos:
- Clonar repositorios
- Configuración automática
- Compartir con equipo

### Extensions (Beta)

Extensiones de terceros:
- Disk usage analyzer
- Logs explorer
- Resource monitor
- Snyk security scanning

## Configuración y Settings

### General

**Opciones principales:**
- ☑️ Start Docker Desktop when you log in
- ☑️ Use Docker Compose V2
- Choose theme (Light/Dark/System)

### Resources

#### **CPU**
- Asignar número de núcleos
- Recomendado: 2-4 CPUs para desarrollo

#### **Memory**
- Asignar RAM dedicada
- Recomendado: 4-8 GB para desarrollo
- Cuidado: no asignar más del 75% de tu RAM total

#### **Disk**
- Ubicación de imágenes y contenedores
- Tamaño máximo del disco virtual
- Opción: Clean / Purge data

#### **File Sharing** (macOS/Windows)
- Directorios compartidos entre host y contenedores
- Por defecto: home directory
- Añadir rutas adicionales si necesario

### Docker Engine

Editor JSON de configuración del daemon:
```json
{
  "builder": {
    "gc": {
      "enabled": true,
      "defaultKeepStorage": "20GB"
    }
  },
  "experimental": false,
  "features": {
    "buildkit": true
  }
}
```

### Kubernetes

**Habilitar Kubernetes local:**
- ☑️ Enable Kubernetes
- Esperar instalación (primera vez: 5-10 minutos)
- kubectl configurado automáticamente

**Uso:**
```bash
# Verificar que Kubernetes está activo
kubectl cluster-info
kubectl get nodes

# Contexto docker-desktop
kubectl config current-context
```

**Ventajas:**
- Cluster local para testing
- Sin configuración adicional
- Reseteable desde Settings

**Desventajas:**
- Consume recursos (2GB+ RAM)
- Single-node cluster
- No recomendado para producción

### Extensions

**Instalar extensiones:**
- Browse Marketplace
- Click "Install"
- Reinicia Docker Desktop si necesario

**Extensiones populares:**
- **Disk usage:** Analizar espacio usado
- **Logs Explorer:** Buscar en logs
- **Resource Usage:** Monitor avanzado

## Integración con Docker Compose

### Desde la UI

1. **Ver stacks de Compose**
   - Pestaña "Containers"
   - Contenedores agrupados por proyecto

2. **Acciones de grupo**
   - Start/Stop todo el stack
   - Ver logs combinados
   - Delete completo

### Desde Terminal Integrada

Docker Desktop incluye terminal integrada:
- Settings → Command Line
- Acceso rápido a docker y docker-compose

## Uso con VS Code

### Docker Extension para VS Code

**Instalación:**
1. Abre VS Code
2. Extensions → Busca "Docker"
3. Instala "Docker" (Microsoft)

**Características:**
- Explorador de contenedores/imágenes/registries
- Click derecho para acciones comunes
- Generación automática de Dockerfiles
- Debug de contenedores

**Workflow:**
```bash
# En VS Code:
1. Abrir proyecto
2. Cmd/Ctrl + Shift + P
3. "Docker: Add Docker Files to Workspace"
4. Seguir wizard
5. F5 para debug
```

## Mejores Prácticas

### Gestión de Recursos

1. **No sobre-asignar recursos**
   - Deja recursos para el sistema operativo
   - Ajusta según necesidad real

2. **Limpieza regular**
   ```bash
   # Desde terminal
   docker system prune -a

   # Desde UI
   Settings → Resources → Disk → Clean / Purge data
   ```

3. **Monitorea espacio en disco**
   - Dashboard muestra uso total
   - Instala extensión "Disk usage"

### Seguridad

1. **Actualiza regularmente**
   - Docker Desktop notifica actualizaciones
   - Settings → Software Updates

2. **No ejecutes como root** (Linux)
   - Usa rootless mode cuando sea posible

3. **Escanea imágenes**
   - Click derecho en imagen → "Scan image"
   - Instala extensión Snyk

### Rendimiento

#### Windows (WSL 2)
- Coloca proyectos dentro de WSL filesystem (`\\wsl$\Ubuntu\home\...`)
- No uses filesystem de Windows (`C:\...`) para mejor rendimiento

#### macOS
- Usa VirtioFS (habilitado por defecto en versiones recientes)
- Limita File Sharing solo a directorios necesarios

#### Linux
- Considera usar Docker Engine nativo para máximo rendimiento
- Docker Desktop en Linux es más pesado (usa VM)

## Troubleshooting Común

### Docker Desktop no inicia

**Windows:**
```powershell
# Verificar WSL
wsl --status

# Reiniciar WSL
wsl --shutdown
# Luego reinicia Docker Desktop
```

**macOS:**
```bash
# Resetear configuración
rm -rf ~/Library/Group\ Containers/group.com.docker
```

**Linux:**
```bash
# Verificar servicio
systemctl --user status docker-desktop

# Reiniciar
systemctl --user restart docker-desktop
```

### Contenedores no se conectan a internet

1. Verifica configuración DNS
   - Settings → Docker Engine
   ```json
   {
     "dns": ["8.8.8.8", "8.8.4.4"]
   }
   ```

2. Reinicia Docker Desktop

### Rendimiento lento

1. **Aumenta recursos** (Settings → Resources)
2. **Limpia datos no usados**
   ```bash
   docker system prune -a --volumes
   ```
3. **Desactiva Kubernetes** si no lo usas

### Error "No space left on device"

```bash
# Ver espacio usado
docker system df

# Limpiar
docker system prune -a --volumes

# O desde UI: Settings → Resources → Disk → Clean
```

## Comparación: CLI vs Docker Desktop

| Tarea | CLI | Docker Desktop |
|-------|-----|----------------|
| Ver contenedores | `docker ps` | Click en Containers |
| Ver logs | `docker logs -f <id>` | Click → Logs tab |
| Abrir terminal | `docker exec -it <id> bash` | Click → Terminal |
| Ver estadísticas | `docker stats` | Click → Stats tab |
| Gestionar imágenes | `docker images`, `docker rmi` | Images tab → Delete |
| Búsqueda Hub | `docker search` | Search bar → Pull |
| Ver uso disco | `docker system df` | Dashboard o Extension |

**Recomendación:**
- Usa CLI para automatización y scripts
- Usa GUI para exploración y debugging visual

## Alternativas a Docker Desktop

### ¿Por qué considerar alternativas?

- **Licencia:** Docker Desktop requiere licencia de pago para empresas grandes (>250 empleados o >$10M revenue)
- **Recursos:** Docker Desktop consume más recursos
- **Control:** Alternativas dan más control fino

### Opciones

#### 1. Docker Engine (Linux nativo)
```bash
# Instalar solo Docker Engine
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh
```

**Pros:** Ligero, rápido, gratis
**Cons:** Sin GUI, solo Linux

#### 2. Rancher Desktop
- GUI similar a Docker Desktop
- Gratis y open source
- Usa containerd o dockerd
- Kubernetes incluido

#### 3. Podman Desktop
- Alternativa completa a Docker
- Rootless por defecto
- Compatible con comandos Docker

#### 4. Colima (macOS)
```bash
brew install colima
colima start
```

**Pros:** Ligero, gratis
**Cons:** Solo CLI, solo macOS

## Conclusión

Docker Desktop es ideal para:
✅ Desarrollo local
✅ Principiantes en contenedores
✅ Necesidad de GUI intuitiva
✅ Equipos pequeños (<250 empleados)
✅ Integración con IDEs

Considera alternativas si:
⚠️ Empresa grande (licencia comercial)
⚠️ Recursos limitados
⚠️ Necesitas máximo rendimiento
⚠️ Solo trabajas con CLI

## Recursos Adicionales

- [Documentación oficial](https://docs.docker.com/desktop/)
- [Release notes](https://docs.docker.com/desktop/release-notes/)
- [Docker Desktop vs Docker Engine](https://docs.docker.com/desktop/faqs/general/)
- [Extensiones Marketplace](https://hub.docker.com/search?q=&type=extension)
