...
El crecimiento del uso de contenedores es el resultado de varias tecnologías del kernel de Linux que ya existian desde hace años, trabajando juntas. En esta sección vamos a profundizar en las tecnologías que hacen posible el aislamiento y la gestión eficiente de recursos en los contenedores.

---
**📺 Slide 2:** cgroups (Control Groups)
...
Imagina que tienes un servidor con 32GB de RAM y 8 CPUs. Sin cgroups, una aplicación mal programada podría consumir toda la memoria y dejar sin recursos a las demás aplicaciones. Con cgroups, puedes decir: "Esta aplicación puede usar como máximo 2GB de RAM y el 25% de una CPU, y ni un byte más". La gestion de recursos a ese nivel normalmente no la hacemos porque requiere un conocimiento muy profundo a nivel de cgroups y la gestion en linea de comandos es tediosa, pero es algo que al usar Docker como motor de contenedores, especialmente en orquestadores como kubernetes, lo haremos practicamente por defecto.
...
1. **CPU:**
   ...
   - Ejemplo: "Este proceso puede usar máximo el 50% de una CPU"

2. **Memoria:**
   - Establecer límites de RAM
   - Ejemplo: "Este proceso puede usar máximo 512MB de RAM"
   - Si el proceso intenta usar más, el kernel lo mata (OOM - Out Of Memory)

3. **Disco I/O:**
   - Limitar la velocidad de lectura/escritura en disco
   - Ejemplo: "Este proceso puede leer máximo 10MB/s del disco"

4. **Red:**
   - Controlar el ancho de banda de red
   - Ejemplo: "Este proceso puede usar máximo 100Mbps"

5. **PIDs (Process IDs):**
   - Limitar el número de procesos hijos que puede crear un proceso
...
**¿Por qué es útil?**
- Esto hace que cada contenedor cree que es el único en el sistema
...
Los namespaces tambien permiten aislar estos mecanismos de comunicación entre procesos de forma que elijamos que procesos pueden comunicarse entre si.
...
Los contenedores usan sistemas de archivos de unión como **UnionFS**, **OverlayFS** o **AUFS**, aunque generalmente usaremos **OverlayFS**, que es el que se usa por defecto en docker, para crear el filesystem del contenedor de forma eficiente.
...
---
**📺 Slide 4:** Union Filesystem (UnionFS)
**📺 Slide 5:** Linux Capabilities
**📺 Slide 6:** SELinux / AppArmor
**📺 Slide 7:** Seccomp (Secure Computing Mode)
**📺 Slide 8:** ¿Cómo trabajan juntas?
**📺 Slide 9:** VMs vs Contenedores (nivel técnico)
**📺 Slide 10:** Resumen - Las Tecnologías Clave
